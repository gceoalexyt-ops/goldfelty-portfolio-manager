...
The `portable.exe` builds run without installing. The `.zip` and `.tar.gz`
builds are the same app without an installer, for putting somewhere yourself.

AppImages need the execute bit before they will run:

```bash
chmod +x 'Goldfelty Portfolio Manager-1.0.0-x86_64.AppImage'
```
